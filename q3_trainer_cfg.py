    ...
    def sample(self, labels=None, cfg_scale=3., n_steps=None, set_seed=False):
        ...
        with torch.no_grad():
            ...
            for t_ in tqdm(range(n_steps)):
                ...
                lambda_prime_t = self.diffusion.get_lambda(t - 1)

                # TODO: Add linear interpolation between unconditional and conditional preidiction according to 3 in Algo. 2 using cfg_scale
                # raise NotImplementedError

                epsilon_tild_t = (1 + cfg_scale) * self.eps_model(z_t, labels) - cfg_scale * self.eps_model(z_t, None)

                # TODO: Get x_t then sample z_t from the reverse process according to 4. and 5. in Algo 2.
                # raise NotImplementedError

                x_t = (z_t - self.diffusion.sigma_lambda(lambda_t) * epsilon_tild_t) / self.diffusion.alpha_lambda(
                    lambda_t)

                z_t = self.diffusion.p_sample(z_t, lambda_t, lambda_prime_t, x_t)
